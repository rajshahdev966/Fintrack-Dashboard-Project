...
export let allTransUpdate = () => {
  ...
  allTrans.forEach((transcation, index) => {
    ...
    transTable.innerHTML += `<tr class="hover:bg-(--surface-bright) transition ">
        ...
        <span class="hover:text-(--danger) cursor-pointer" onclick="delTrans(${index}, updateAfterTranscation)"><i class="ri-close-circle-fill"></i></span>
        <span class="hover:text-(--primary) cursor-pointer" onclick="editTrans(${index}, updateAfterTranscation)"><i class="ri-pencil-fill"></i></span>
        </td>
        </tr>`;
  });
};
